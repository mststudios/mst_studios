...
export const CookieBanner: React.FC<CookieBannerProps> = ({ isOpen, onClose }) => {
  ...
  const handleAcceptAll = async () => {
    try { await submitCookieConsent('accepted') } catch {}
    saveConsent(true, true);
  };
  const handleRejectAll = async () => {
    try { await submitCookieConsent('rejected') } catch {}
    saveConsent(false, false);
  };
  const handleSavePreferences = async () => {
    try { await submitCookieConsent('necessary') } catch {}
    saveConsent(preferences.statistics, preferences.marketing);
  };
