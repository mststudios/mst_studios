...
export const PrivacyModal: React.FC<PrivacyModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-brand-bg backdrop-blur-md animate-in fade-in duration-300">
      
      <div 
        className="bg-brand-bg w-full max-w-2xl max-h-[90vh] rounded-[2rem] shadow-2xl border border-brand-border flex flex-col animate-in zoom-in-95 duration-300 relative shadow-none overflow-hidden"
      ...
      >
        ...
        <button 
          onClick={onClose} 
          className="absolute top-6 right-6 p-2 hover:bg-white/10 rounded-full transition-colors text-brand-muted hover:text-white z-20"
        >
          <X className="w-5 h-5" />
        </button>

        {/* Header Section */}
        <div className="p-8 pb-6 bg-brand-bg border-b border-brand-border relative shrink-0">
          <div className="absolute top-0 right-0 w-64 h-64 bg-brand-card rounded-full blur-[80px] -z-10 pointer-events-none"></div>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-brand-card flex items-center justify-center shadow-lg shadow-none border border-brand-border">
          ...
          <div className="space-y-8 text-brand-muted leading-relaxed">
            ...
            <div>
              <h3 className="text-white font-bold text-lg mb-4 flex items-center gap-2">
                <FileText className="w-5 h-5 text-white" /> Hvilke data indsamler vi?
              </h3>
              <ul className="grid sm:grid-cols-2 gap-3">
                {['Navn', 'E-mailadresse', 'Telefonnummer', 'Beskeder via formular'].map((item, idx) => (
                  <li key={idx} className="flex items-center gap-3 bg-brand-bg p-3 rounded-lg border border-brand-border text-sm">
            ...
            <div>
              <h3 className="text-white font-bold text-lg mb-4 flex items-center gap-2">
                <Database className="w-5 h-5 text-white" /> Hvad bruger vi data til?
              </h3>
              <p className="text-sm mb-3">Dine oplysninger bruges udelukkende til følgende formål:</p>
              <ul className="space-y-2 text-sm pl-4 border-l-2 border-brand-border">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-white font-bold text-lg mb-3 flex items-center gap-2">
                  <Lock className="w-5 h-5 text-white" /> Opbevaring
                </h3>
                <p className="text-xs">
              ...
              <div>
                <h3 className="text-white font-bold text-lg mb-3 flex items-center gap-2">
                  <Share2 className="w-5 h-5 text-white" /> Deling
                </h3>
                <p className="text-xs">
            ...
            <div className="bg-brand-card p-6 rounded-2xl border border-brand-border">
              <h3 className="text-white font-bold text-lg mb-4 flex items-center gap-2">
                <UserCheck className="w-5 h-5 text-white" /> Dine Rettigheder (GDPR)
              </h3>
              <div className="grid gap-4 text-sm">
                <div className="flex gap-3">
                  <Eye className="w-5 h-5 text-brand-subtle shrink-0" />
                  <div>
                ...
                <div className="flex gap-3">
                  <X className="w-5 h-5 text-brand-subtle shrink-0" />
                  <div>
            ...
            <div className="border-t border-brand-border pt-6 mt-2">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <a href={`mailto:${EMAIL_ADDRESS}`} className="flex items-center gap-3 text-brand-muted hover:text-white transition-colors group">
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-white/5 transition-colors">
                    <Mail className="w-5 h-5" />
                  </div>
                  <span className="text-sm font-medium">{EMAIL_ADDRESS}</span>
                </a>
                <a href={`tel:${PHONE_NUMBER.replace(/\s/g, '')}`} className="flex items-center gap-3 text-brand-muted hover:text-white transition-colors group">
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-white/5 transition-colors">
        ...
        <div className="p-6 bg-brand-bg border-t border-brand-border flex justify-end">
          <button 
            onClick={onClose}
            className="px-6 py-3 bg-brand-card hover:bg-brand-card text-white text-xs font-bold uppercase tracking-widest rounded-xl transition-all border border-brand-border hover:border-white/30"
          >
            Luk vindue
          </button>
        </div>
